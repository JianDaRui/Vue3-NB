...
前面在回顾部分，我们已经通过console观察了`defineComponent`的返回内容，其内部实现就如其表现：
...
接下来让我们一起看下`defineAsyncComponent`的源码：

- 解构`source` & 定义`load`函数
- `load`函数的主要职责就是去加载用户传入的异步组件
  - `load`函数首先会进行异步处理，如何异步处理下面说，这里先不聊
  - 加载成功会`resolve`异步组件
...
- 接上面没有说的异常处理：
  - defineAsyncComponent 允许用户配置onError函数，进行异步处理
  - onError函数会可以接受四个参数，回顾部分我们有说
  - 异常处理是在调用loader函数的catch中进行的
